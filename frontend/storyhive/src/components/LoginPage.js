...
export default function LoginPage({ onSwitchLoginClick, submit }) {
  const navigate = useNavigate(); // Initialize useNavigate

  // This function handles form submission and fetches the username
  const handleSubmit = (e) => {
    e.preventDefault();
    const fetched_userName = e.target[0].value; // Grab the username from the form
    submit(fetched_userName); // Pass the username to the submit function (if needed for additional functionality)

    const data = {
      // Store the contents of the form
      name: e.target.uname.value,
      pass: e.target.pword.value,
    };
    ...
    fetch("https://localhost:10000/check-form", options)
      .then((response) => response.json())
      .then((json) => {
        alert(json.status);
        if (json.status === "Login successful!") {
          navigate("/profile"); // Redirect to profile page on successful login
          localStorage.setItem("elementData", e.target.uname.value); // Store username in localStorage
        } else {
          alert(json.status); // Show error if login fails
        }
      });
  };
    ...
    <div class="w-[1920px] h-[1239px] relative bg-[#bf6a02] border border-white">
      <div class="w-[1920px] h-[1239px] left-0 top-0 absolute border border-black">
        ...
        >
          ...
          <div class="left-[215px] top-[33px] absolute text-[#111111] text-xl font-bold font-['Poppins'] lowercase">
            SIGNUP
          </div>
          <div class="w-[198px] h-[95px] left-0 top-0 absolute bg-[#fff1c2] rounded-[100px]"></div>
          <div class="left-[60px] top-[25px] absolute text-[#222222] text-3xl font-bold font-['Poppins'] lowercase">
            LOGIN
          </div>
        </button>

        <div class="w-[771px] h-[820px] left-[575px] top-[244px] absolute rounded-[100px] bg-white shadow-xl"></div>
        <div class="w-[607px] h-[60px] left-[701px] top-[409px] relative pl-7 text-black text-[40px] font-bold font-['Poppins']">
          Welcome back to Storyhive
          <br />
        </div>

        <form id="login-form1" onSubmit={handleSubmit}>
          {/* username bubble and input */}
          <div class="w-[584px] h-[126px] left-[669px] top-[420px] relative">
            <label class="w-full h-full" for="uname">
              <input
                class="rounded-[100px] border-2 border-[#aaaaaa] text-black text-2xl w-full h-full pl-10 placeholder:text-2xl placeholder:italic"
                type="text"
                id="uname"
                name="uname"
        ...
        <a
          href="#"
          class="w-[269px] h-[59px] left-[1005px] top-[385px] relative text-black text-[25px] font-normal font-['Poppins'] underline lowercase"
        >
          FORGOT PASSWORD
        </a>
